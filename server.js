// ============================================================
// draw-report-relay  (v3 — botoes Banir/Desbanir + orientacao configuravel)
//   Roblox  -> POST /report (secret)         -> posta a denuncia (com botoes)
//   Discord -> POST /interactions (assinado) -> Banir/Desbanir via Open Cloud
// Requer Node 18+.
// ============================================================
...
const BAN_DURATION_SECONDS = parseInt(process.env.BAN_DURATION_SECONDS || "0", 10);
const BAN_DISPLAY_REASON = process.env.BAN_DISPLAY_REASON || "Banido pela moderacao.";

const FLIP_X = (process.env.FLIP_X ?? "1") !== "0";
const FLIP_Y = (process.env.FLIP_Y ?? "1") !== "0";

const DISCORD_API = "https://discord.com/api/v10";
const app = express();

app.get("/", (_req, res) => res.send("draw-report-relay v3 ok"));
...
app.post("/report", express.json({ limit: "8mb" }), async (req, res) => {
    try {
      ...
      const ts = Number(report.timestamp) || Math.floor(Date.now() / 1000);
          ...
          const authorName = String(report.authorName ?? "?").slice(0, 32);
          const embed = {
      ...
      let ok;
          if (BOT_TOKEN && CHANNEL_ID) {
                  const components = [{
                            type: 1,
                            components: [
                              { type: 2, style: 4, label: "🔨 Banir autor", custom_id: `ban:${authorId}:${authorName}`.slice(0, 100) },
                              { type: 2, style: 2, label: "♻️ Desbanir", custom_id: `desban:${authorId}:${authorName}`.slice(0, 100) },
                                      ],
                  }];
                  ok = await postViaBot({ embeds: [embed], components }, png);
          } else if (WEBHOOK_URL) {
                  ok = await postViaWebhook({ username: "Moderacao", embeds: [embed] }, png);
          } else {
...
app.post("/interactions", express.raw({ type: "*/*" }), async (req, res) => {
    ...
    let verified = false;
    try {
          verified = nacl.sign.detached.verify(Buffer.concat([Buffer.from(ts), raw]), Buffer.from(sig, "hex"), Buffer.from(PUBLIC_KEY, "hex"));
    ...
    if (interaction.type === 1) return res.json({ type: 1 });
           ...
           if (interaction.type === 3) {
                 ...
                 const isBan = customId.startsWith("ban:");
                 const isUnban = customId.startsWith("desban:");
                 if (!isBan && !isUnban) return res.json({ type: 4, data: { flags: 64, content: "Acao desconhecida." } });
      ...
      const roles = interaction.member?.roles || [];
                 ...
                 const allowed = isAdmin || (MOD_ROLE_IDS.length > 0 && roles.some(r => MOD_ROLE_IDS.includes(r)));
                 if (!allowed) return res.json({ type: 4, data: { flags: 64, content: "❌ Voce nao tem permissao." } });

      const parts = customId.split(":");
                 const userId = parts[1];
                 const name = parts.slice(2).join(":") || userId;
                 const result = await setBan(userId, isBan, `Discord (mod ${clicker})`);
                 if (result.ok) {
                         const verbo = isBan ? "banido" : "desbanido";
                         return res.json({ type: 4, data: { content: `${isBan ? "🔨" : "♻️"} **${name}** (\`${userId}\`) foi **${verbo}** por <@${clicker}>.` } });
                 }
                 return res.json({ type: 4, data: { flags: 64, content: `⚠️ Falha (HTTP ${result.status}): ${result.detail}`.slice(0, 1800) } });
           }
...
async function postViaBot(payload, png) {
    ...
    const r = await fetch(`${DISCORD_API}/channels/${CHANNEL_ID}/messages`, { method: "POST", headers: { Authorization: `Bot ${BOT_TOKEN}` }, body: form });
...
async function setBan(userId, active, privateReason) {
    if (!ROBLOX_API_KEY || !ROBLOX_UNIVERSE_ID) return { ok: false, status: 0, detail: "ROBLOX_API_KEY/UNIVERSE_ID nao configurados" };
    const gameJoinRestriction = { active: active, privateReason: String(privateReason).slice(0, 400), excludeAltAccounts: false };
    if (active) {
          gameJoinRestriction.displayReason = String(BAN_DISPLAY_REASON).slice(0, 400);
    ...
    try {
          const r = await fetch(url, { method: "PATCH", headers: { "x-api-key": ROBLOX_API_KEY, "Content-Type": "application/json" }, body: JSON.stringify({ gameJoinRestriction }) });
          const text = await r.text().catch(() => "");
          if (!r.ok) { console.error("[relay] roblox setBan", active, r.status, text.slice(0, 400)); return { ok: false, status: r.status, detail: text.slice(0, 280) }; }
...
function buildSvg(strokes) {
    ...
    const mx = (x) => ((FLIP_X ? (maxX - x) : (x - minX)) * scale).toFixed(2);
    const my = (y) => ((FLIP_Y ? (maxY - y) : (y - minY)) * scale).toFixed(2);
    const out = [
...
app.listen(PORT, () => console.log(`[relay] v3 ouvindo na porta ${PORT} (FLIP_X=${FLIP_X}, FLIP_Y=${FLIP_Y})`));
